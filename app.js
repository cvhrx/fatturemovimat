...
// Local-only PWA (v28): no prev/next buttons
const KEY='spese_records_local_v7';
...
function renderMonth(){
  ...
  for(let d=new Date(first); d<=last; d.setDate(d.getDate()+1)){
    const wd=d.getDay(); if(wd===0||wd===6) continue; // skip weekend
    ...
    const p2=document.createElement('div'); p2.className='pill '+(sforato?'warn':'ok'); p2.textContent='Tot/2 '+euro(tot2);
    const p3=document.createElement('div'); p3.className='pill '+(trasf?'pill-perno':'pill-gior'); p3.textContent=trasf?'Pernottamento':'Giornaliera';
    const p4=document.createElement('div'); p4.className='pill '+(residuo>=0?'ok':'warn'); p4.textContent='Residuo '+euro(residuo);

    pills.append(p1,p2,p3,p4); head.append(title,pills);
  ...
  const left = monthBudget - monthTotal;
  const leftEl = byId('month_left');
  leftEl.textContent = (left>=0?'Residuo mese: ':'Sforamento mese: ') + euro(Math.abs(left));
  leftEl.className = 'badge ' + (left>=0?'good':'warn');
}
